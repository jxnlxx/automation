...
for i in site_list.index:
    ...
    for item in kw_list:
        ...
        try:
            temp = pd.DataFrame({
                ...
                'google_rank' : item.get('KeywordRanking').get('Google').get('Rank')
                }, index=[0])
    ...
    print(f'Completed {k} of {total_kws}')
    drop = df[df['created_at'] > cutoff ].index
    df = df.drop(drop)
    df = df.reset_index(drop=True)
    print(f'Saving {save_name}.csv...')
    df.to_csv(fr'L:\Commercial\Operations\Technical SEO\Automation\STAT\Data\SERP Trends\Keywords List\{save_name}_keywords_list_after_2019-06-01.csv', index=False)
