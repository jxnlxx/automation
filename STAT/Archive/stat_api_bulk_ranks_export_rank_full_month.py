# -*- coding: utf-8 -*-
"""
Created on Mon Jun 22 16:52:29 2020
...
# =============================================================================
    ...
    try: # try loading existing storage for client. 
        df = pd.read_csv(fr'L:\Commercial\Operations\Technical SEO\Automation\Setup\Client Ranks\Historical\Exports\Google Base Rank\{site_url}_rank.csv')
    ...
    for i in range(len(row)):
        ...
        if pd.isnull(site_ids): # checks if site_ids is empty - if it is, try next cell in row
            continue
        else:
...
# =============================================================================
    ...
    df.to_csv(fr'L:\Commercial\Operations\Technical SEO\Automation\Setup\Client Ranks\Historical\Exports\Google Base Rank\{site_url}_rank.csv', index=False)
    print('\n'+f'Updating {year}_{month:02d}_bulk_ranks_all.csv...')
    jobs_all.at[f'{site_url}','rank_status'] = 'Done' # add 'Done to 'Status' col for URL
