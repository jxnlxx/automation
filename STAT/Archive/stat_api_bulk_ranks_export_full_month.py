...
try:
    ...
    jobs_head = jobs_head.drop('Status', axis=1)
#    jobs_head = jobs_head.head(10)
## if new month chop off head(10), then add ['Status'] column to jobs_all
except KeyError:
#    jobs_head = jobs_all.head(10)
    jobs_all['Status'] = ''
    jobs_all['Status'] = jobs_all['Status'].astype(str) # convert keyword_id to str so merge works properly...
print('Done!')
...
# =============================================================================
    ...
    try: # try loading existing storage for client. 
        df = pd.read_csv(fr'L:\Commercial\Operations\Technical SEO\Automation\Setup\Client Ranks\Historical\Exports\Google Base Rank\{site_url}_base_ranks.csv')
...
# =============================================================================
    ...
    df.to_csv(fr'L:\Commercial\Operations\Technical SEO\Automation\Setup\Client Ranks\Historical\Exports\Google Base Rank\{site_url}_base_ranks.csv', index=False)
