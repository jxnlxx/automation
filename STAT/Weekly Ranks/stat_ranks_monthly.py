#%% stat_weekly_ranks_transformation.py
...
#   get end_date from requests table
    ...
    for j in getstat.mon_weekspan(start_date, end_date):
        ...
        print(f"Starting period {sDate} - {eDate}...")

        sql = f"""
            SELECT Ranks_{save_name}.Date, KeywordsTable.Keyword, KeywordsTable.RegionalSearchVolume, Ranks_{save_name}.BaseRank as Rank, KeywordDevice.Device, KeywordCategory1.Category as Category1, KeywordCategory2.Category as Category2
...
for i in client_list.index:
    ...
    if client_name in multiple_categories:
        sql = f"""
        SELECT Visibility_{save_name}.Date, KeywordDevice.Device as Device, Criteria.Criterion as Criteria, KeywordCategory1.Category as Category1, KeywordCategory2.Category as Category2, Visibility_{save_name}.Score
        FROM Visibility_{save_name}
        JOIN KeywordDevice ON Visibility_{save_name}.DeviceId = KeywordDevice.Id
        JOIN Criteria ON Visibility_{save_name}.CriterionId = Criteria.Id
        JOIN KeywordCategory1 ON Visibility_{save_name}.KeywordCategory1Id = KeywordCategory1.Id
        JOIN KeywordCategory2 ON Visibility_{save_name}.KeywordCategory2Id = KeywordCategory2.Id
        ...
        sql = f"""
        SELECT Visibility_{save_name}.Date, KeywordDevice.Device as Device, Criteria.Criterion as Criteria, KeywordCategory1.Category as Category, Visibility_{save_name}.Score
        FROM Visibility_{save_name}
        JOIN KeywordDevice ON Visibility_{save_name}.DeviceId = KeywordDevice.Id
        JOIN Criteria ON Visibility_{save_name}.CriterionId = Criteria.Id
        JOIN KeywordCategory1 ON Visibility_{save_name}.KeywordCategory1Id = KeywordCategory1.Id
