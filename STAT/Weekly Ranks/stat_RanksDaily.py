...
client_list = client_list[client_list["Status"] == "Complete"]
...
#   request reports for between "start_date" and "end_date"
    ...
    for j in pd.date_range(start_date, end_date):
        ...
        print(f"{n} Requesting {date} rank report for {client_name}")
...
# retrieve reports
    ...
    for j in requests_df.index:
        ...
        print(f"\n{j+1} of {len(requests_df)} - Fetching {client_name} data for {date}")
        ...
        if len(new_kws) > 0: # if new keywords, add them to database without tags
            ...
            for x in new_kws.index:
                ...
                try:
                    TargetedSearchVolume = int(new_kws["KeywordStats.RegionalSearchVolume"][x])
                except KeyError:
                    TargetedSearchVolume = int(new_kws["KeywordStats.TargetedSearchVolume"][x])
            ...
            # Category5
                ...
                VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?,  ?, ?, ?, ?)""",
                (StatId, Keyword, MarketId, LocationId, DeviceId, CreatedAt, GlobalSearchVolume, TargetedSearchVolume, Jan, Feb, Mar, Apr, May, Jun, Jul, Aug, Sep, Oct, Nov, Dec, Category1Id,  Category2Id, Category3Id, Category4Id, Category5Id)
                )
