...
tags = df2.loc[:,df2.columns.str.contains('category')].columns # create list of new columns (allows # of columns to vary)
...
for i in tags:
    ...
    for j in groups.index:
        ...
        if tag not in [np.nan, 'nan', '']: # skip empty tags
            ...
            match = df1['keyword'].str.contains(substrings, regex=True, case=False, na=False)
