...
import getstat as stat
from getstat import stat_subdomain, stat_key, stat_base_url                                    # saved in venv/Lib
...
for i in client_list.index:
    ...
    save_name = pace.scrub(client_name)
...
#   get date of next request
    ...
    #       see if start date is before 1 years ago
        start_date = pace.cutoff(start_date)
...
#   request reports for between 'start_date' and 'end_date'
    ...
    for j in pd.date_range(start_date, end_date):
        ...
        job_id = stat.bulkranks(stat_id, date)
