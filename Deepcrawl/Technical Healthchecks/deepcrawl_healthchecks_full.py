...
n = 1
if os.path.exists(fr'{templates_path}\Backup\{backup_month}'):
    pass
else:
    print('Creating backup of data...')
    os.mkdir(templates_path + fr'\Backup\{backup_month}')
    src_dir = templates_path
    dst_dir = templates_path + fr'\Backup\{backup_month}'
    time.sleep(1)
...
for i in hc_list.index:
    ...
    try:
        df_data = pd.read_excel(templates_path + fr'\{client} - Tech Healthcheck Template.xlsx', sheet_name='Data')
        df_healthcheck = pd.read_excel(templates_path + fr'\{client} - Tech Healthcheck Template.xlsx', sheet_name='Healthcheck')

    except FileNotFoundError:
        df_data = pd.read_excel(templates_path + fr'\_Blank Tech Healthcheck Template.xlsx', sheet_name='Data', read_only=True)
        df_healthcheck = pd.read_excel(templates_path + fr'\_Blank Tech Healthcheck Template.xlsx', sheet_name='Healthcheck', read_only=True)
...
for i in hc_list.index:
    ...
    try:
        df = openpyxl.load_workbook(filename=templates_path + fr'\{client} - Tech Healthcheck Template.xlsx', data_only=True)
        df = df['Healthcheck']
    ...
    wb = xl.Workbook(teams_path + fr'\{client} - Tech Healthcheck.xlsx')
